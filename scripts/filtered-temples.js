...
const homeLink = document.querySelector("#displaysall");
homeLink.addEventListener("click", () => {
  createTempleCard(temples);
});


const smallLink = document.querySelector("#smaller");
smallLink.addEventListener("click", () => {
createTempleCard(temples.filter(temple => temple.area < (10000)));
});

const largeLink = document.querySelector("#larger");
largeLink.addEventListener("click", () => {
  createTempleCard(temples.filter(temple => temple.area > (90000)));
});


// This Function to extract the year from a date string (e.g., "2008, November, 18",)
function extractYear(dateString) {
  const date = new Date(dateString); // Create a Date object from the string
  return date.getFullYear(); //This Gets the full year
}
...
const newLink = document.querySelector("#before");
newLink.addEventListener("click", () => {
  // Filter temples where the dedication year is before 1900 and pass to function createTempleCard to createTempleCard
  const filteredTemples = temples.filter(temple => extractYear(temple.dedicated) < 1900);
  createTempleCard(filteredTemples); 
});
...
function createTempleCard(filteredTemples) {
  ...
  filteredTemples.forEach(temple => {
    ...
    document.querySelector(".filter").appendChild(card);
  });
}